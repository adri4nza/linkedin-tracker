...
export interface DailyResultsDrawerProps {
  ...
  data: GameRecord[];
  /**
   * Optional. When provided (i.e. this drawer was opened from another panel),
   * the header shows a "← Back" button instead of the close (✕) button.
   * The backdrop click still calls onClose as usual.
   */
  onBack?: () => void;
}
        ...
        <div className="bg-white/40 dark:bg-slate-900/30 px-5 py-4 border-b border-slate-200/60 dark:border-slate-700/50 shrink-0">
          <div className="flex items-start justify-between">
            <div className="flex items-start gap-2">
              {/* Back button — only when coming from another drawer */}
              {onBack && (
                <button
                  onClick={onBack}
                  className="p-1.5 mt-0.5 rounded-xl hover:bg-slate-100/80 dark:hover:bg-slate-800/60 active:scale-95 transition-all duration-300 shrink-0"
